 ...
 * {@code AbstractCraftingUnitBlock.onRemove() -> CraftingBlockEntity.breakCluster()} the broken position is
 * <b>already air</b> while its block entity is still alive and <b>not</b> removed, and the cluster's
 * block-entity list has not been rebuilt yet. Any code that walks {@code cluster.getBlockEntities()} and calls
 * {@code getUnitBlock()} inside that window gets
 * {@code ClassCastException: AirBlock cannot be cast to AbstractCraftingUnitBlock}, which is thrown straight
 * out of {@code onRemove()} and crashes the server with "Ticking GridNode".
 *
 ...
 *
 * <p>Shared verbatim with the 1.21.1 target: this only uses {@code BlockEntity.isRemoved/getLevel/getBlockPos}
 * and {@code Level.isLoaded/getBlockState}, all of which exist in 1.20.1 with the same signatures.
 */
