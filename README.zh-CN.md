...
| Minecraft | 1.21.1 |
| NeoForge | 21.1.250 |
| Applied Energistics 2 | 19.2.16 及以上（**必需前置**，缺它不加载） |
| Java | 21 |

> **关于 AE2 版本。** 下限是 **19.2.16**——AE2 在这一版加入了作业挂起（PR #8635），而"冻结 CPU"正是建立在它之上。
> 下限写在模组元数据里，所以跑更老 AE2 的包会从加载器那里得到一条明确的依赖提示（在本模组任何代码运行之前），
> 而不是在 AE2 加载合成逻辑时崩掉。本模组挂钩的其余成员从 19.2.0-beta 起就存在，所以这个下限是由**用到的
> 最新成员**决定的，而不是由**本可以兼容的最老版本**决定的。
