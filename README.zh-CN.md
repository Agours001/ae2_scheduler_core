...
| Minecraft | 1.21.1 |
| NeoForge | 21.1.250 |
| Applied Energistics 2 | 19.2.0 及以上（**必需前置**，缺它不加载）；冻结 CPU 需要 19.2.16 及以上 |
| Java | 21 |

> **关于 AE2 版本。** 本模组的全部功能从 AE2 19.2.0 起就可用。唯一例外是"冻结 CPU"——它建立在 AE2 自己的
> 作业挂起标记之上，而那个标记是 19.2.16 才加的（PR #8635）。因此在更早的 19.2.x 上，模组会**正常加载并运行，
> 只是关掉这一个功能**，而不是拒绝启动：老版本 AE2 的包失去的是一个按钮，不是整个模组。那些版本的 AE2 界面上
> 本来也没有挂起按钮，两边是一致的。
...
**有接口、没有功能**的目标（本目标跑在 19.2.16 之前的 AE2 上就是这种情况）什么都不装配，同时由混入插件把
命名了那些成员的**两个混入整体撤掉**，游戏里不会留下"挂了一半"的钩子。（判定由 `SuspendApiProbe` 完成：它把
AE2 的类文件**当资源读**，因为在应用混入配置的阶段不允许加载类；而"字段名出现在它自己的类文件里"恰好就是要问
的那个问题。）调度器于是把每一单都当作可运行——这也正是"没有挂起功能的 AE2"该有的含义。
