...
> **先做 1.21.1，再向下移植到 1.20.1。** 1.21.1 是本项目的最初目标，也是参考实现；1.20.1 是把同一份共享核心
> 移植到 AE2 15.x 的结果。有一项能力在移植中无法保留——AE2 直到 **19.2.16** 才加入作业**挂起**，所以 1.20.1
> 线上没有冻结按钮，所有订单一律视为可运行。除此之外两条线跑的是同一份代码。
...
- **两种页面，互不干扰**——订单行是该订单的页面；**CPU 行是"整台机器"的页面**：图标是调度核心方块，进度是所有订单的合计。点选订单**不会**改变 CPU 页显示的内容。
- **逐单取消**——只取消你选中的那一单，它没用掉的原料还回网络。
- **逐单挂起/恢复**——挂起是真正的开关，不是单向操作。
- **一键冻结整台 CPU**——在 CPU 自己的页面上（CPU 行，或 CPU 方块自己的界面），挂起按钮一次作用于**全部订单**，而且状态随订单存档：冻结的 CPU 重启后仍然冻结。
...
| Applied Energistics 2 | 19.2.16 及以上 | 15.4.0 及以上（构建于 15.4.10） |
| Java | 21 | 17 |

> **关于 1.21.1 线的 AE2 下限。** 下限是 **19.2.16**——AE2 在这一版加入了作业挂起（PR #8635），而"冻结 CPU"正是
> 建立在它之上。下限写在模组元数据里，所以跑更老 AE2 的包会从加载器那里得到明确的依赖提示（在本模组任何代码运行
> 之前），而不是在 AE2 加载合成逻辑时崩掉。本模组挂钩的其余成员从 19.2.0-beta 起就存在，所以下限由**用到的
> 最新成员**决定。
>
> **关于 1.20.1 线。** 那边没有这个问题：AE2 15.x 从未加入作业挂起，所以那份构建完全没有冻结功能——没有标记，
> 也没有界面按钮——调度器就把每一单都当作可运行，其余共用同一份代码。那一边真正需要的是 **MixinExtras**：
> Forge 1.20.1 与 AE2 都不提供，所以本模组把它打包进自己的 jar。
...
| `mc1_20_1/` | 目标：MC 1.20.1 / Forge 47.4.x（同时可跑 NeoForge 47.1.x）。拥有自己的入口、方块与物品、客户端模型、本代"无参 `writeToNBT`"的 accessor，以及自己的 mod 元数据。由 ModDevGradle 的 `legacyforge` 插件构建：ForgeGradle 6 在现代 JDK 上无法配置，而这样两个目标用的是同一套工具链。 |
...
1.20.1 目标正是这个拆分存在的理由：AE2 15.x 完全没有作业挂起——没有标记，也没有界面按钮——所以那个目标既不
包含命名它的混入与 accessor，也不装配 `SuspendSupport` 实现。调度器于是把每一单都当作可运行，这正是那一代的
含义。1.21.1 目标声明 AE2 19.2.16+，并装配对应的 accessor。

| | |
|---|---|
| JDK | `mc1_21_1` 用 21、`mc1_20_1` 用 17（各目标以 toolchain 钉死）；Gradle 自身跑在 21 上 |
...
合成状态界面里的**每一行本身就是一个 `ICraftingCPU`**：菜单遍历 `ICraftingService.getCpus()` 生成行，用对象身份分配行号，点击某行时又把**同一个对象**交回来。服务端没有别的办法往这个列表里加行，所以每个订单都得到一个自己的轻量适配器（`SchedulerJobCpu`）；点中适配器那一行，就设定了挂起与取消按钮读取的**焦点**——那是"该订单的页面"。而 **CPU 自己那一行是另一个主语**：它代表整台机器，[它汇报的内容](#cpu-行汇报什么)也是机器的。
...
列表里 **CPU 那一行**描述的是**整台机器**，而且它是唯一被这样固定住的一行：

| | |
|---|---|
| 图标 | **调度核心方块**。它标记"这一行是这台 CPU 的总视图"，同时说明这台 CPU 是装了调度核心的。用某一单的产物当图标是假的（机器并不是在做一个订单），而且会随订单进出而变。 |
| 进度 / ETA | 各订单进度**按订单量加权**后的合计：AE2 的进度是分数，所以权重决定了"大单拉动进度条更多"。已用时间取最长那个订单的。 |

**其余一切都跟随你选中的那一行**，因为它们都是"页面"：订单行是**该订单的页面**（物品表、等待列、待输出列、进度与 ETA 都是它自己的计划），而没有选中任何行时，页面描述的是机器——整台 CPU 的共享库存与合计，也就是原版自己的合并口径。

CPU 行刻意避开了两条规则，1.0.4 之前它们都是真实行为：跟随**被聚焦**的订单（机器那一行会汇报你刚点的那一单），以及跟随**当前时间片**的订单（每 tick 换一个，进度会每次刷新都乱跳）。合计是唯一稳定的说法——而**一行不是页面**，所以它不该像页面那样跟随选择。

### 一个按钮，两个主语

挂起按钮会看自己身处哪个页面：选中了某个订单行时，它切换**那一单**；在 CPU 自己的页面上（CPU 行，或**没有列表的** CPU 方块界面）它切换**这台 CPU 上的全部订单**，一键冻结、再按一键释放。状态按订单存档（`ExecutingCraftingJob.suspended`，由 AE2 自己的序列化器写入），所以冻结的 CPU 重启后仍然冻结。

要说清"冻结"**不**做什么：准入逻辑没有改动，所以往冻结中的 CPU 提交新订单仍会被收下并立刻开始跑——"挂起全部"停住的是**已存在的订单**，不是给机器装了个总闸。
...
| `/schedulercore uiprobe rows` | 原样打印 `ICraftingService.getCpus()`，含每行画出来的**图标**（逐单行到底能不能上屏；CPU 行是不是调度核心方块 + 合计进度） |
| `/schedulercore uiprobe focus <id>` / `release` | 等价于"点了某订单那一行" / "回到整台 CPU 页" |
| `/schedulercore uiprobe cancel` | 等价于"按了取消按钮"（有聚焦则只取消该单，否则取消全部） |
| `/schedulercore uiprobe toggle [vanilla]` | 等价于"按了挂起按钮"（有聚焦则作用于该单，无聚焦则作用于全部订单），并打印前后状态 |
...
2. **共享原料显示的是整台 CPU 的总量，不是某一单的份额。** 订单页会隐藏属于别的订单的行，但它显示出来的数量仍是共享池的——AE2 不保留"哪些料属于哪个订单"的账。
...
7. **逐单行只存在于"合成状态界面"里。** 右键合成 CPU 打开的那个界面**根本没有 CPU 列表**——那是 AE2 的布局，想加一个就等于自己写 GUI。所以它那两个按钮作用于整台机器：**挂起**会冻结或释放该 CPU 上的**全部**订单，**取消**会取消该 CPU 上的全部订单。要控制单个订单，请在合成终端的 **Status（状态）** 页选中它的那一行。
